...
# =========================
def convert_interface_to_short_form(interface: str) -> str:
    """Convert verbose interface names to short form"""
    if not interface:
        return interface
    
    # Cisco interface conversions
    interface = re.sub(r'^GigabitEthernet', 'Gi', interface)
    interface = re.sub(r'^TenGigabitEthernet', 'Te', interface)
    interface = re.sub(r'^FastEthernet', 'Fa', interface)
    interface = re.sub(r'^Ethernet', 'Eth', interface)
    interface = re.sub(r'^Port-channel', 'Po', interface)
...
STATIC_LOOKUP = {
    ...
    "aruba-mp-edge-sw1-a.ucl.ac.uk": "10.17.10.52",
...
GATEWAY_ROUTERS = {
    ...
    'aruba-mp-edge-sw1-a.ucl.ac.uk' : r'172.17.(40|41).\d+'
}
...
class PathHop:
    ...
    def to_dict(self):
        """Convert PathHop to dictionary format with short-form interfaces"""
        return {
            'device_name': self.device_name,
            'device_ip': self.device_ip,
            'local_interface': convert_interface_to_short_form(self.local_interface)
        }
...
# =========================
# Trace Path with Source Interface Discovery
# =========================
def trace_path_with_interfaces(start_ip: str, target_name: str, creds: dict):
    visited_hostnames = set()
    queue = deque()
    
    # Create the source hop (interface will be discovered from LLDP)
    source_device_name = start_ip
    for name, ip in STATIC_LOOKUP.items():
        if ip == start_ip:
            source_device_name = name
            break
    
    start_hop = PathHop(source_device_name, start_ip)
    queue.append((start_ip, source_device_name, [start_hop]))

    log_debug(f"➡️ Starting trace from {start_ip} ({source_device_name})")
    ...
    while queue:
        ...
        for candidate in candidates:
            ...
            # If this is the source device, update its interface from LLDP
            if current_ip == start_ip and path[0].local_interface is None:
                path[0].local_interface = local_intf
                log_debug(f"✅ Updated source interface from LLDP: {local_intf}")
...
# =========================
def convert_path_to_dict_format(path: list) -> list:
    """Convert PathHop objects to dictionary format with short interfaces"""
    dict_path = []
    for hop in path:
        if hop.device_ip:  # Include all hops with IP
            dict_path.append(hop.to_dict())
    return dict_path

def print_simple_path_format(path: list):
    """Print path in simple comma-separated format"""
    print("\nSimple path format:")
    for hop in path:
        if hop.device_ip:
            if hop.local_interface:
                short_interface = convert_interface_to_short_form(hop.local_interface)
                print(f"'{hop.device_name}','{hop.device_ip}','{short_interface}'")
            else:
                print(f"'{hop.device_name}','{hop.device_ip}',''")
    print()

def get_trunk_interfaces_from_path(path: list) -> list:
    """Extract trunk interfaces with proper short-form conversion"""
    trunk_list = []
    for hop in path:
        if hop.local_interface and hop.device_ip:
            ...
            trunk_list.append({
                ...
                'local_interface': short_interface
            })
    return trunk_list
...
if __name__ == "__main__":
    ...
    dict_path = convert_path_to_dict_format(path1)
    print(f"\nPath in dictionary format:")
    print(dict_path)
    
    # Print simple format
    print_simple_path_format(path1)
    
    trunk_interfaces = get_trunk_interfaces_from_path(path1)
    print(f"Local interfaces to check for VLAN tags: {trunk_interfaces}")
    log_debug(f"Local interfaces to check for VLAN tags: {trunk_interfaces}")
    
    print(f"\nTrace complete. Debug log: {DEBUG_FILE}")